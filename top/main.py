...
def main():
    ...
    ControlTimeout()
    SendEmail(f'Top OK: elapsed time is {int(time.time() - START)}')
...
def ControlTimeout():
    ...
    if (current - START) > TIMEOUT:
        ...
        SendEmail(f'Top FAIL: elapsed time is {int(current - START)}, while allowed is {TIMEOUT}!')
        sys.exit()
    else:
...
def PrepareData(raw: dict, sheet_name: str, column_names: list):
    try:
        ...
        height = 0
        print(Fore.LIGHTMAGENTA_EX + datetime.now().strftime('[%m-%d|%H:%M:%S] ') + f'For sheet {sheet_name} found NO operations!' + Style.RESET_ALL)
    else:
        print(Fore.GREEN + datetime.now().strftime('[%m-%d|%H:%M:%S] ') + f'For sheet {sheet_name} found {height} operations.' + Style.RESET_ALL)
...
def Authorize(session: requests.Session):
    print(Fore.LIGHTBLUE_EX + datetime.now().strftime('[%m-%d|%H:%M:%S] ') + f'Trying to authorize TopVTop URL: {URL_AUTH}...' + Style.RESET_ALL)
    try:
        response = session.post(URL_AUTH, headers=HEADERS_AUTH, data=DATA_AUTH, cookies=COOKIES_AUTH)
    except requests.ConnectionError:
        print(Fore.RED + datetime.now().strftime('[%m-%d|%H:%M:%S] ') + f'Connection error on authorization!' + Style.RESET_ALL)
        Sleep(LONG_SLEEP)
        session = Authorize(session)
    else:
        if response.status_code == 200:
            print(Fore.GREEN + datetime.now().strftime('[%m-%d|%H:%M:%S] ') + f'Success status = {response.status_code} on authorization.' + Style.RESET_ALL)
        else:
            print(Fore.RED + datetime.now().strftime('[%m-%d|%H:%M:%S] ') + f'Error status = {response.status_code} on authorization!' + Style.RESET_ALL)
            Sleep(LONG_SLEEP)
            session = Authorize(session)
    return session
...
def GetData(session: requests.Session):
    print(Fore.LIGHTBLUE_EX + datetime.now().strftime('[%m-%d|%H:%M:%S] ') + f'Trying to connect TopVTop URL: {URL_DATA}...' + Style.RESET_ALL)
    try:
        response = session.get(URL_DATA, headers=HEADERS_GET, params=PARAMS_GET, cookies=COOKIES_GET)
    except requests.ConnectionError:
        print(Fore.RED + datetime.now().strftime('[%m-%d|%H:%M:%S] ') + f'Connection error on TopVTop URL: {URL_DATA}!' + Style.RESET_ALL)
        Sleep(LONG_SLEEP)
        raw = GetData(session)
    else:
        if response.status_code == 200:
            print(Fore.GREEN + datetime.now().strftime('[%m-%d|%H:%M:%S] ') + f'Success status = {response.status_code} on TopVTop URL: {URL_DATA}.' + Style.RESET_ALL)
            raw = response.json()
        else:
            print(Fore.RED + datetime.now().strftime('[%m-%d|%H:%M:%S] ') + f'Error status = {response.status_code} on TopVTop URL: {URL_DATA}!' + Style.RESET_ALL)
            Sleep(LONG_SLEEP)
            raw = GetData(session)
    return raw
