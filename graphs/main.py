...
def Main():
    ...
    for i, (cabinet, cookie) in enumerate(CABINETS_AND_COOKIES.items()):
        CleanSheet(len(COLUMNS), cabinet, SHEET_ID, service)
        final_data = []
        column = chr(i + ord('A'))
        date_from, date_to = GetColumn(column, service, 'Periods', SHEET_ID)
        spec_codes = GetColumn(column, service, 'Numbers', SHEET_ID)
        for code in spec_codes:
            data = GetData(cookie, code, date_from, date_to)
...
@ControlRecursion
def GetData(cookie: dict, code: str, date_from: str, date_to: str) -> dict:
    ...
    try:
        response = requests.post(URL, headers=HEADERS, json=body, cookies=cookie)
    except requests.ConnectionError:
        Stamp(f'Connection on {URL}', 'e')
        Sleep(LONG_SLEEP)
        raw = GetData(cookie, code, date_from, date_to)
    else:
